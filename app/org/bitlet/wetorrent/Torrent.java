...
public class Torrent extends InterruptableTasksThread {
    ...
    public List<Map> trackerRequest(String event) {
            ...
            for (Tracker t : trackers) {
                try {
                    ...
                    if (responseDictionary != null) {
                        ...
                        trackers.remove(t);
                        trackers.add(0, t);
                        returnList.add(responseDictionary);
                    }
                } catch (Exception e) {
                    int ciccio = 2;
                    e.printStackTrace();
    ...
    public void tick() {
        ...
        if (!stopped) {
            try {
                for (Map peerMap : trackerRequest(null)) {
                    Object peers = peerMap.get(toByteBuffer("peers"));
                    if (peers != null) {
                        addPeers(peers);
                    }
                }
    ...
    public void startDownload() throws Exception {
        stopped = false;
        List<Map> firstResponseDictionary = trackerRequest("started");

        if (firstResponseDictionary == null) {
            throw new Exception("Problem while sending tracker request");
        }
        for (Map peerMap : firstResponseDictionary) {
            Object peers = peerMap.get(toByteBuffer("peers"));
