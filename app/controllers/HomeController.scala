...
/**
  ...
  */
@Singleton
class HomeController @Inject()(seriesDAO: SeriesDAO, seasonDAO: SeasonDAO, episodeDAO: EpisodeDAO)(implicit ec: ExecutionContext) extends Controller {
  ...
  def uncheck(id:Long) = Action {
    Await.result(seriesDAO.uncheckRequested(id),Duration.Inf)
    Redirect(routes.HomeController.index())
  }

  def check(id:Long) = Action {
    Await.result(seriesDAO.checkRequested(id),Duration.Inf)
    Redirect(routes.HomeController.index())
  }
}
