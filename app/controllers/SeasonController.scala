...
class SeasonController @Inject()(seriesDAO: SeriesDAO, seasonDAO: SeasonDAO, episodeDAO: EpisodeDAO)(implicit ec: ExecutionContext) extends Controller {
    ...
    episodeDAO.episodesOfId(idSeason).map { episodes =>
      Await.result(seriesDAO.findById(idSeries), Duration.Inf) match {
        case Some(series) => Await.result(seasonDAO.findById(idSeason), Duration.Inf) match {
          case Some(season) => Ok(views.html.season(series, season, episodes, idSeries, idSeason))
  ...
  def uncheck(id: Long, seriesId: Long, seasonId: Long) = Action {
    Await.result(episodeDAO.uncheckRequested(id), Duration.Inf)
    Redirect(routes.SeasonController.show(seriesId, seasonId))
  }

  def check(id: Long, seriesId: Long, seasonId: Long) = Action {
    Await.result(episodeDAO.checkRequested(id), Duration.Inf)
    Redirect(routes.SeasonController.show(seriesId, seasonId))
  }
}